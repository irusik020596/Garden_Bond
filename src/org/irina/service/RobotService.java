...
// GET	
	   ...
	   {
		   return RestDAO.addRobot(user, password, lotId, name, description, type, status, state);
	   }	
	   @GET
	   @Path("/edit")
	   @Produces("text/plain")
	   ...
	   {
		   return RestDAO.editRobot(user, password, id, name, description, type, status, state);
	   }	
	   @GET
	   @Path("/delete")
	   @Produces("text/plain")
